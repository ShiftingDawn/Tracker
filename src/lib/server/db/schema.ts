...
export const userQuestCompletionTable = pgTable("user_quest_completion", {
  ...
  uniqueIndex("user_quest_completion.unique_user_id_quest_id").on(table.userId, table.questId),
]);
...
export type UserQuestCompletion = typeof userQuestCompletionTable.$inferSelect;

export const userQuestPinnedTable = pgTable("user_quest_pinned", {
  userId: uuid("user_id").notNull().references(() => userTable.id),
  questId: uuid("quest_id").notNull().references(() => gameQuestTable.id),
  order: smallint("order").notNull(),
  createdAt: timestamp("created_at", {withTimezone: true, mode: "date",}).notNull().defaultNow(),
  ...
  primaryKey({columns: [table.userId, table.questId,],}),
  uniqueIndex("user_quest_pinned.unique_user_id_quest_id").on(table.userId, table.questId),
]);
export const userQuestPinnedRelations = relations(userQuestPinnedTable, ({one,}) => ({
  user: one(userTable, {fields: [userQuestPinnedTable.userId,], references: [userTable.id,],}),
